...
def get_phone_region(html):
    ...
    phoneRegionList = [x.replace('<', '').replace('>', '') for x in phoneRegionList]
    return phoneRegionList


def join_number_region(html):
    regionDict, phoneRegionList = get_city_province(html), get_phone_region(html)
    phoneNumRegionList = []
...
if __name__ == '__main__':
    ...
    with toolkit_sqlite.SqliteDB(DB_FILE) as sqlitedb:
        ...
        df.to_sql(tableName, con=sqlitedb.conn, index=False, if_exists='append')
